...
if (!isset($_GET['reload'])) {
    header("refresh:0;url=../ViewReporte/baja.php?reporte=baja&reload=1");
}
...
if (isset($_GET['recarga'])) {
    ...
    if (strval($_GET['recarga'])  != "adentro") {
        ...
        header("refresh:1;url=../ViewReporte/baja.php?reporte=baja&reload=1&recarga=1");
    }
}
...
<?php if (isset($_GET['reload'])) { ?>
    ...
    <body style="overflow: hidden;">
        ...
        <div class="overflow-auto" style="padding-top: 20px; height: 100vh ; " class="col">
            ...
            <button style="margin-bottom:10px ;" onclick="exportTableToExcel('#tabla', 'InformedeBaja.xlsx' )" class="btn btn-outline-primary">
            ...
            <table id="tabla" class=" table table-sm  table-bordered ">
                ...
                <tbody>
                    <?php
                    $cantidad = 0;
                    while ($row = $baja->fetchArray(SQLITE3_ASSOC)) {
...
<script>
    function adentro() {
        ...
        if (recarga == "1") {
            location.replace("../ViewReporte/baja.php?reporte=baja&recarga=adentro&reload=1");
        }
    }

    function afuera() {

        location.replace("../ViewReporte/baja.php?reporte=baja&recarga=1&reload=1");
